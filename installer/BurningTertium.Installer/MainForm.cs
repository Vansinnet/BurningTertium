...
namespace BurningTertium.Installer;
...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        var heading = new Label { Text = "BurningTertium", Font = new Font(Font.FontFamily, 22, FontStyle.Bold), AutoSize = true };
        var description = new Label
        {
            Text = "Turns the Mourningstar mission-table hologram of Tertium red and sets its rooftops on fire. " +
                   "Safe installer for Steam build 24735202 / Darktide 1.3.770.210: it changes four hologram " +
                   "materials (with backups) and installs the BurningTertium DMF mod. " +
    ...
    private async Task Run(InstallerAction action)
    {
        if (action == InstallerAction.RepairAfterUpdate && MessageBox.Show(this,
                "Use this after a Darktide update has overwritten BurningTertium files. " +
                "The installer will accept a changed game version only when every managed file has a known safe hash. " +
                "If Fatshark changed a required bundle, it will stop before writing and require a new BurningTertium release.",
                "BurningTertium update repair", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) != DialogResult.OK)
            return;
        SetBusy(true);
        ...
        try
        {
            ...
            MessageBox.Show(this, result.Message, "BurningTertium", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        catch (Exception error)
        {
            status.Text = error.Message;
            MessageBox.Show(this, error.Message, "BurningTertium stopped safely", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
