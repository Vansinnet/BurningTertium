...
namespace BurningTertium.Installer.Core;
...
public sealed class InstallerEngine
{
        ...
        Action<int>? beforeWrite = null)
    {
        ...
        this.stateRoot = Path.GetFullPath(stateRoot ?? Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "BurningTertium"));
    ...
    public PayloadManifest LoadManifest()
    {
        ...
        if (manifest.Format != 1 || manifest.Product != "BurningTertium" || manifest.Files.Count == 0)
            throw new InstallerException("Unsupported or empty BurningTertium payload manifest.");
        ...
        string observedBuild)
    {
        ...
        if ((action is InstallerAction.Repair or InstallerAction.RepairAfterUpdate) && oldReceipt is null)
            throw new InstallerException("BurningTertium is not installed by this installer; repair cannot establish ownership.");
        ...
        if ((action is InstallerAction.Repair or InstallerAction.RepairAfterUpdate) &&
            ...
            throw new InstallerException("The ownership receipt belongs to a different BurningTertium release.");
        ...
        var stageRoot = Path.Combine(gameRoot, ".burningtertium-stage", session);
        ...
        try
        {
            foreach (var recipe in manifest.Files)
            {
                ...
                if (existingKnownOutput && prior is null)
                    throw new InstallerException($"Refusing to adopt an existing BurningTertium output without an ownership receipt: {recipe.Target}");
            ...
            {
                InstallerAction.Repair => "BurningTertium repair completed and all files were verified.",
                InstallerAction.RepairAfterUpdate =>
                    $"BurningTertium was restored after Steam build {observedBuild}. All managed file hashes were compatible and verified.",
                _ => "BurningTertium installation completed. Backups were preserved."
            };
    ...
    private static void PreflightUpdateRepair(string gameRoot, PayloadManifest manifest, InstallReceipt receipt)
    {
        ...
        foreach (var recipe in manifest.Files)
        {
            ...
            else if (!output)
            {
                if (!exists || new FileInfo(target).Length != recipe.BaseSize ||
                    ...
                    throw new InstallerException($"Update repair stopped before writing: {recipe.Target} changed in this Darktide update. A new BurningTertium release is required.");
            }
        }
        ...
        _ = LoadOrder.Add(File.ReadAllBytes(loadOrder), "BurningTertium");
        ...
        ref int writeNumber, ReceiptFile? old)
    {
        ...
        var updated = LoadOrder.Add(original, "BurningTertium");
    ...
    {
        var receiptPath = ReceiptPath(gameRoot);
        if (!File.Exists(receiptPath)) throw new InstallerException("No BurningTertium ownership receipt was found.");
        ...
        try
        {
            ...
            RemoveEmptyOwnedDirectories(gameRoot);
            return new OperationResult(true, "BurningTertium was uninstalled. Original backups remain preserved.");
        }
        catch
        {
            Rollback(journal, journalPath);
    ...
    {
        var modRoot = Path.Combine(gameRoot, "mods", "BurningTertium");
        if (!Directory.Exists(modRoot)) return;
        Safety.RejectReparseAncestors(gameRoot, modRoot, true);
        var allowed = manifest.Files.Where(file => file.Target.StartsWith("mods/BurningTertium/", StringComparison.OrdinalIgnoreCase))
        ...
        var previouslyOwned = receipt?.Files
            .Where(file => file.Target.StartsWith("mods/BurningTertium/", StringComparison.OrdinalIgnoreCase))
            .ToDictionary(file => Path.GetFullPath(Path.Combine(gameRoot, file.Target.Replace('/', Path.DirectorySeparatorChar))),
                StringComparer.OrdinalIgnoreCase);
        foreach (var path in Directory.EnumerateFiles(modRoot, "*", SearchOption.AllDirectories))
        {
            ...
            if (!isAllowed || !currentOutput && !priorOutput)
                throw new InstallerException($"Refusing unknown file in the BurningTertium mod directory: {path}");
        }
        ...
        ReceiptFile? installed)
    {
        ...
        if (installed?.Backup is not null && installed.BackupSha256 is not null)
        {
            ...
            var installedFromBackup = LoadOrder.Add(backup, "BurningTertium");
            if (current.AsSpan().SequenceEqual(installedFromBackup)) original = backup;
        }
        var updated = original ?? LoadOrder.Remove(current, "BurningTertium");
    ...
    private static void RestoreBackup(string target, string backup, long size, string hash)
    {
        ...
        var stage = Path.Combine(Path.GetDirectoryName(target)!, ".bt-restore-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var stage = Path.Combine(Path.GetDirectoryName(target)!, ".bt-stage-" + Guid.NewGuid().ToString("N"));
    ...
    {
        foreach (var relative in new[] { "mods/BurningTertium/scripts/mods/BurningTertium", "mods/BurningTertium/scripts/mods", "mods/BurningTertium/scripts", "mods/BurningTertium" })
        {
