...
public static class GameDiscovery
{
    ...
    public static void VerifyDependencies(string root)
    {
        if (!Directory.Exists(Path.Combine(root, "binaries", "mod_loader")) &&
            ...
            throw new InstallerException("Darktide Mod Loader (DML) is missing. Install DML before BurningTertium.");
        var dmf = Path.Combine(root, "mods", "dmf", "dmf.mod");
        if (!File.Exists(dmf))
            throw new InstallerException("Darktide Mod Framework (DMF) is missing. Install DMF before BurningTertium.");
    }
}
