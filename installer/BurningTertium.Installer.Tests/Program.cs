...
{
    using var fixture = new Fixture();
    var path = Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin");
...
static void OwnedVersionUpgrade()
{
    ...
    Equal("new-v2", File.ReadAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "upgraded addition");
    ...
    False(File.Exists(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "upgrade uninstall addition");
}
...
static void InterruptedVersionUpgradeRollback()
{
    ...
    Equal("new", File.ReadAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "upgrade addition rollback");
...
static void InterruptedRollback()
{
    ...
    False(File.Exists(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "addition rollback");
}
...
static void Repair()
{
    using var fixture = new Fixture();
    fixture.Engine().Execute(fixture.Game, InstallerAction.Install, false);
    File.Delete(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin"));
    fixture.Engine().Execute(fixture.Game, InstallerAction.Repair, false);
    Equal("new", File.ReadAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "repair output");
}
...
static void UpdateRepairCompatible()
{
    ...
    Equal("new", File.ReadAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "update addition");
}
...
static void UpdateRepairMixedState()
{
    ...
    Equal("new", File.ReadAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "mixed existing addition");
}
...
static void UpdateRepairChangedReplacementPreflight()
{
    ...
    var engine = fixture.Engine();
    engine.Execute(fixture.Game, InstallerAction.Install, false);
    var addition = Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin");
...
static void UpdateRepairChangedAdditionPreflight()
{
    ...
    engine.Execute(fixture.Game, InstallerAction.Install, false);
    File.WriteAllText(Path.Combine(fixture.Game, "base.bin"), "base");
    File.WriteAllText(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin"), "unknown");
...
static void UpdateRepairRequiresMatchingRelease()
{
    ...
    Throws(() => engine.Execute(fixture.Game, InstallerAction.RepairAfterUpdate, false), "different BurningTertium release");
...
{
    var root = Path.Combine(Path.GetTempPath(), "BurningTertium-version-test-" + Guid.NewGuid().ToString("N"));
...
static void Uninstall()
{
    ...
    False(File.Exists(Path.Combine(fixture.Game, "mods", "BurningTertium", "new.bin")), "uninstall addition");
...
static void LoadOrderPreservation()
{
    ...
    var added = LoadOrder.Add(original, "BurningTertium");
    Equal("dmf\r\nOther\r\nBurningTertium\r\n", Encoding.UTF8.GetString(added), "append preserving CRLF");
    var duplicate = LoadOrder.Add(added, "BurningTertium");
    True(added.AsSpan().SequenceEqual(duplicate), "idempotent load order");
    Equal("dmf\r\nOther\r\n", Encoding.UTF8.GetString(LoadOrder.Remove(added, "BurningTertium")), "remove preserving content");
}
...
{
    public string Root { get; } = Path.Combine(Path.GetTempPath(), "BurningTertium-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public Fixture()
    {
        ...
        var manifest = new PayloadManifest
        {
            ...
            Files = new List<FileRecipe>
            {
                ...
                Recipe("addition", "mods/BurningTertium/new.bin", null, null, "new", true, "inserts/addition.bin")
            }
        };
